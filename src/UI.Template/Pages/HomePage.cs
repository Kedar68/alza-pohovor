...
public class HomePage() : BaseEshopPage("/")
{
    ...
    public bool AddToBasketFirstProductFromCategory(string category)
    {
        ...
        Dictionary<string, CheckoutCard> productCards = _productsGrid.GetProductCards();
    ...
    public ProductDetailPage OpenProductByNameFromCategory(string category, string product)
    {
        ...
        if (!productCards.TryGetValue(product, out CheckoutCard? value))
        {
    ...
    /// <returns></returns>
    public bool TryGetProductCardByName(string productName, out CheckoutCard productCard)
    {
        Dictionary<string, CheckoutCard> productCards = _productsGrid.GetProductCards();

        if (!productCards.TryGetValue(productName, out CheckoutCard? value))
        {
            productCard = null!;
            return false;
        }
